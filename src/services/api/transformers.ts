...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const billableUsage = raw['claude-billable-usage'] ?? raw.claudeBillableUsage;
  if (isRecord(billableUsage)) {
    config.claudeBillableUsage = {
      enabled: normalizeBoolean(billableUsage.enabled),
    };
  }
